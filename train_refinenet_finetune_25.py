...
# Init wandb
wandb.init(project="LiverTumor", name="RefineNet_FineTuning_Top25")
...
# Build model (all frozen initially)
model = build_refinenet(trainable=False)

# Load weights from transfer learning (optional, but good practice)
...
checkpoint = ModelCheckpoint("refinenet_finetune_top25_best_model.h5", monitor="val_loss", save_best_only=True)
...
model.save("refinenet_finetune_top25_final_model.h5")
wandb.finish()
