...
# Initialize wandb
wandb.init(project="LiverTumor", name="AttentionUNet_TransferLearning")
...
def attention_gate(skip, gating, inter_channels):
    ...
    out = tf.keras.layers.Multiply()([skip, sigmoid])
    return out
...
def build_attention_unet(input_shape=(256, 256, 1)):
    ...
    u3 = tf.keras.layers.concatenate([u3, att3])
...
checkpoint = ModelCheckpoint(
    "attention_unet_transfer_best_model.h5",
    monitor='val_loss',
    save_best_only=True
)
...
model.save("attention_unet_transfer_final_model.h5")
wandb.finish()
