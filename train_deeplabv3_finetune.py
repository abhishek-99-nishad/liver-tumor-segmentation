...
from model_deeplabv3 import build_deeplabv3

# =====================
# Initialize wandb
# =====================
wandb.init(project="LiverTumor", name="DeepLabV3_FineTuning")
...
# =====================
# Build DeepLabV3 (Unfrozen backbone)
# =====================
model = build_deeplabv3(input_shape=(256,256,1), trainable=True)

# Load transfer weights
model.load_weights("deeplabv3_transfer_best_model.h5")
...
checkpoint = ModelCheckpoint(
    "deeplabv3_finetune_best_model.h5",
    monitor='val_loss',
    save_best_only=True
)
...
history = model.fit(
    ...
    epochs=10,
    batch_size=8,   # keep low for CPU stability
...
# Save Final Model
# =====================
model.save("deeplabv3_finetune_final_model.h5")
